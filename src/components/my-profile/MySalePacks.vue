  ...
  >
    ...
    <v-card>
      <v-toolbar dark color="primary" class="sticky-toolbar">
        <v-toolbar-title>Gói dịch vụ đã bán</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-toolbar-items>
        ...
        >
          <template v-slot:items="{ items }">
            <template v-for="pack in items">
              <v-card class="mb-4 pa-4" :key="pack.id">
                <div class="title primary--text mb-2">GÓI#{{ pack.id }}</div>
                <div
                  class="body-1 d-flex align-end justify-space-between caption"
                >
                  <div class="font-weight-bold">Khách hàng</div>
                  <v-divider class="mb-1"></v-divider>
                  <div>{{ pack.customer.name }}</div>
                </div>
                <div
                  class="
                    body-1
                ...
                >
                  ...
                  <v-divider class="mb-1"></v-divider>
                  <div>{{ pack.total_price | currencyFormat }}</div>
                </div>
                <div class="body-1 d-flex align-end justify-space-between">
                  ...
                  <div class="font-weight-bold green--text">
                    {{ pack.sale_commission | currencyFormat }}
                  </div>
                </div>
              </v-card>
            </template>
